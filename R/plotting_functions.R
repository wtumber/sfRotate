...
transpose_bbox <- function(original_bbox) {
  ...
  sf::st_bbox(c(xmin=-((abs(xmin)+abs(xmax))/2),
                ymin = -((abs(ymin)+abs(ymax))/2),
                xmax=((abs(xmin)+abs(xmax))/2),
                ymax=((abs(ymin)+abs(ymax))/2)))
}
...
create_boundary_shape <- function(vertices = 24,radius = 1,transpose = c(0,0)){
  ...
  message(paste("If you are creating a square you might have wanted radius :\n",sqrt(2*(radius^2))))
